...
4. **Required keys (same rows on both `PROD` and `STAGE`)**  
   ...
   | `password_reset_message` | _(Optional — server default if omitted.)_ e.g. `If an account exists, password reset instructions were sent.` |
   | `sign_up_confirm` | _(Optional default.)_ e.g. `Check your email to confirm your account.` (new signup and duplicate-email signup) |
   | `dup_email_*` | _(Optional defaults.)_ Customize duplicate-email notice; see `BASE_APP_REQUIREMENTS` |
